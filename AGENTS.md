...
Spec: `psychology-education-skills-grok-build-requirements-v1.1.md` (V1.1). Current scope: **M0–M4 complete; M5 (Dynamic Skills Runtime) complete** — M5.1 (canonical skill registry, registry/runtime consistency tests, CI), M5.2 (semantic router, dynamic DAG planner, Legacy Step Adapter), M5.3 (external skill runtime, minimum context, policies, semantic PII) and M5.4 (subagents with strict contracts, parallel DAG scheduler, conversational build layer, UI) are landed (renderer available: LibreOffice at `C:\Program Files\LibreOffice`; ppt-foundry delivery check wired).
