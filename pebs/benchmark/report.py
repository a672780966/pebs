...
def _row_for(run: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_seconds": metrics.get("wall_time_seconds"),
...
def render_markdown(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        f"生成时间：{summary.get('generated_at')}；run 数：{summary.get('runs', 0)}",
        "",
        "| Case | Mode | Human Score | Edit Ratio | Evidence Errors | Routing Errors | Plan Errors | Model Calls | Runtime(s) |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for case in summary.get("cases", []):
        for mode in COLUMNS:
            ...
            if not entry:
                continue
            lines.append(
                "| {case} | {mode} | {human} | {edit} | {evidence} | {routing} | {plan} | {calls} | {runtime} |".format(
                    case=case["case_id"],
                    mode=mode,
